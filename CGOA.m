...
function [TargetFitness,TargetPosition,Convergence_curve,Trajectories,fitness_history, position_history]=CGOA(N, Max_iter, lb,ub, dim, fobj)
...
GrassHopperPositions=crossover(GrassHopperPositions,Pc,etac);
...
while l<Max_iter+1
    ...
    r=(rMin-rMax*l/Max_iter)*(1+pr*(rand-1));
    for i=1:size(GrassHopperPositions,1)
        ...
        if rand<0.5
          ...
          X_new = c * S_i_total' + exp(-r)*(X_new - TargetPosition); % search operator in the pigeon algorithm
        end 
